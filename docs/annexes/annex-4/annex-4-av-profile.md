...
- AVI Invocation is done via the W3C Digital Credentials API or an equivalent platform API. Any other mechanism, including Custom URL schemes, MUST NOT be used
...
- HAIP requires support for signed requests to enable RP authentication through 
mechanisms beyond the Web PKI. This is not a requirement for the Age Verification 
solution; therefore, unsigned requests are used instead.
- HAIP requires encrypted responses, using the response mode `dc_api.jwt`. The threat model of 
the Age Verification solution does not include leaking of responses by the browser or 
the operating system, as both are assumed to be low risk for the specific use case. Moreover, introducing an 
additional encryption layer is deemed unnecessarily complex, offering little protection
against such attacks while outweighing the associated risk. 
