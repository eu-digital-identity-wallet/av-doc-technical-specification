...
* **AVS Unit Attestation (AVSUA)**: A data object, issued by the provider of an Age
Verification solution, that describes an Age Verification App Instance and the device on
which it is installed, enabling an Attestation Provider to verify that the App Instance is
a genuine and untampered instance of a compliant Age Verification solution. It comprises an
Age Verification App Instance Attestation (AVIA), attesting to the integrity of the App
Instance, and a Key Attestation (KA), attesting to the security of the keys it uses. The
AVSUA is the Age Verification instantiation of the Wallet Unit Attestation mechanism
specified in [TS3], in which the AVIA corresponds to the Wallet Instance Attestation; the
[ARF] provides the wider context.
* **Implementer**: An individual who is responsible for translating software
...
| AVIA | Age Verification App Instance Attestation |
| AVS | Age Verification Solution |
| AVSUA | AVS Unit Attestation |
...
| \[TS3\] | [Specification of Wallet Unit Attestations (WUA) used in issuance of PID and Attestations, version 1.5.2, 26 May 2026](https://github.com/eu-digital-identity-wallet/eudi-doc-standards-and-technical-specifications/blob/main/docs/technical-specifications/ts3-wallet-unit-attestation.md) |
...
| Item Reference | Standard name/details |
| :---- | :---- |
| \[ARF\] | [EU Digital Identity Architecture and Reference Framework](https://eudi.dev/3.0.0/)|
| \[OID4VCI\] | [OpenID for Verifiable Credential Issuance, version 1.0](https://openid.net/specs/openid-4-verifiable-credential-issuance-1_0.html) |
...
| \[ISO/IEC CD TS 23220-4\] | [ISO/IEC CD TS 23220-4: Cards and security devices for personal identification — Building blocks for identity management via mobile devices. Part 4: Protocols and services for operational phase.](https://www.iso.org/standard/86785.html) |
...
* **Data protection:**
  ...
  * **Unlinkability:** The goal of the solution is to prevent user profiling
  ...
  will rely on Zero-Knowledge Proof (ZKP) and batch issuance to protect users from colluding RPs.
...
Device (figure 4) and Cross Device flow (figure 5).
...
device than the device the Age Verification App resides on. In this case the
...
interface, enabling customization to meet organisational or national needs.
...
identification information included in a physical identity card or passport,
...
3. Age Attestation Mechanism
   ...
   * Obtain an AVS Unit Attestation (AVSUA) describing the AVI properties
   (see Authentication Workflow below)
...
* The AVI obtains an AVS Unit Attestation (AVSUA) issued by the provider of the Age
Verification solution. The AVSUA comprises an Age Verification App Instance Attestation
(AVIA), describing the AVI and its context, and a Key Attestation (KA), covering the keys
the AVI uses. It is based on the attestation capabilities of the mobile platform (e.g.
Android Key Attestation, iOS App Attest), consumed through the AVSUA mechanism rather than
directly.
* The AVI requests a Proof-of-Age attestation from the AP using OID4VCI, presenting the
AVIA as a client attestation and the KA in the Credential request, as profiled in
[Annex A](annexes/annex-A/annex-A-av-profile.md), Section A.5 (see also the
[Wallet Attestation](https://openid.net/specs/openid-4-verifiable-credential-issuance-1_0.html#section-13.3) authentication method of OID4VCI).
...
Proof of Age attestations presented as a plain ISO mDoc are single-use. Since neither the Attestation Provider nor the Age Verification App Instance can determine, at issuance time, whether the fallback presentation mechanism will be required, the system SHALL support the issuance of attestations in batches so that the User has several attestations available. It is recommended that each
...
It is recommended that the Proof of Age attestation remain valid for a maximum period of three (3) months from the date of issuance. It is recommended that a Proof of Age attestation presented as a plain ISO mDoc be treated as a single-use credential. An attestation presented as a Zero-Knowledge Proof MAY be reused within its validity period, since the proof does not permit linkage across presentations.
If a revocation mechanism is required, a status list may be utilised as an effective solution for managing the
...
Trust in Age Verification Apps is established at issuance time. In the age verification
architecture, issuance is the point at which the entire chain of trust is anchored: a
Relying Party trusts a Proof of Age attestation because it is signed by a listed
Attestation Provider, and the Attestation Provider can only vouch for attestations it
has issued to genuine, untampered instances of compliant Age Verification solutions. If
a repackaged or cloned app, a tampered execution environment, or an emulator farm can
obtain Proof of Age attestations, every downstream safeguard is undermined, including
the large-scale onward supply of valid attestations to minors. Before issuing a Proof
of Age attestation, the Attestation Provider therefore verifies that the requesting app
is a genuine and untampered instance of a compliant Age Verification solution, using
the AVS Unit Attestation mechanism or an internal mechanism of equivalent assurance
(see Sections 4.2 and 4.3, and Annex A, Section A.5, which defines the outcomes any
such mechanism is required to achieve).

Note: The envisaged EU Age Verification Scheme is expected to establish a trusted list
of Age Verification solution providers, which will serve as the trust anchor against
which AVS Unit Attestations are validated. Until that list is established, validation
is anchored in the certificates lodged with the Commission by Age Verification App
providers and published in the list of compliant Age Verification Apps (see Section
4.2).  This interim validation approach will be described in greater detail in future iterations of the specifications.

![Figure 8](./media/Figure_8_trusted-list.png)
...
Proof of Age attestation.
* The trust anchor CA shall comply with [ETSI EN 319 411-1\] under the scope NCP
...
Proof of Age attestation presentation, SHALL implement the Zero-Knowledge Proof
mechanism defined in [Annex A](annexes/annex-A/annex-A-av-profile.md), Section A.8, as the preferred presentation method, SHALL implement the plain ISO mDoc presentation as a fallback for devices that do not support Zero-Knowledge Proofs, and MAY implement the protocols specified
...
* Where a Proof of Age attestation is presented as a plain ISO mDoc, the Age Verification App SHALL use a Proof of Age attestation only once and SHALL then remove it from the batch of the issued attestations. An attestation presented as a
Zero-Knowledge Proof is not consumed and MAY be reused within its validity
period, in accordance with Section 3.4.3.
* An Age Verification App SHOULD support issuance of Proof of Age attestations
following the EUDI Wallet issuance framework specified in the [ARF]. In
particular, an Age Verification App Instance SHOULD be able to obtain an AVS
Unit Attestation (AVSUA) and present it to the
Attestation Provider during issuance, as specified in [Annex A](annexes/annex-A/annex-A-av-profile.md), Section A.5.
* Provider of an Age Verification App compliant with these specifications SHALL inform the Commission about the Age Verification App prior to its publication in the application stores, and SHALL at the same time lodge with the Commission the certificate(s) it uses to sign its AVS Unit Attestations, together with any subsequent changes to them.
* The Commission SHALL maintain a list of Age Verification Apps compliant with these specifications on its website, including the lodged AVS Unit Attestation signing certificates or their fingerprints.
...
the data model set in [Annex A](annexes/annex-A/annex-A-av-profile.md), Section A.4.
...
* An Attestation Provider SHALL verify, before issuing a Proof of Age
attestation, that the requesting app is a genuine and untampered instance of a compliant
Age Verification solution, and SHALL refuse issuance if this verification fails.
The RECOMMENDED mechanism for this verification is the AVS Unit Attestation
(AVSUA) profiled in [Annex A](annexes/annex-A/annex-A-av-profile.md), Section A.5.
* An Attestation Provider SHALL perform this verification against the current list of
compliant Age Verification Apps at each issuance, and SHALL refuse issuance where the
Age Verification solution, or the version of it identified in the verification, has been
withdrawn from the list.
* An Attestation Provider and an Age Verification App Provider MAY use an
internal mechanism instead of the AVSUA (for example, where both roles are
performed by the same or affiliated entities), provided that it achieves the assurance
outcomes defined in [Annex A](annexes/annex-A/annex-A-av-profile.md), Section A.5,
regarding the legitimacy and integrity of the Age
Verification App Instance.
...
* A Relying Party SHALL implement the Zero-Knowledge Proof
verification mechanism specified in [Annex A](annexes/annex-A/annex-A-av-profile.md), Section A.8, and SHALL accept the plain ISO mDoc fallback presentation defined in Section A.6.
...
the main method utilising eID based authentication using delegation to an
eIDAS node or a trusted identity provider.

Additionally, the white label app SHALL include functionality to scan the
ICAO9303 Machine Readable Zone (MRZ) utilising existing open source library
capable of scanning or capturing an image of the data page, enabling
extraction of the MRZ, and following utilising existing open source library
...
issuing service SHALL support batch issuing for the age over nn attestation in accordance with Section 3.4.1.
...
An age verification service that supports
the protocols specified in [Annex A](annexes/annex-A/annex-A-av-profile.md)
using ISO mDoc age over nn attestation SHALL be available. The verification service SHALL support verification of the Zero-Knowledge Proof mechanism defined in [Annex A](annexes/annex-A/annex-A-av-profile.md), Section A.8, as the preferred mechanism, and plain ISO mDoc verification as the fallback. The service SHALL
...
The Technical Specifications adopt the Zero-Knowledge Proof (ZKP) solution described in
the paper *"Matteo Frigo and abhi shelat, Anonymous credentials from ECDSA, IACR Communications in Cryptology, vol. 3, no. 1, 4 May 2026, doi: [10.62056/a3qjmpgxq](https://doi.org/10.62056/a3qjmpgxq). Preprint: Cryptology ePrint Archive, Paper 2024/2010, 2024, available at [https://eprint.iacr.org/2024/2010](https://eprint.iacr.org/2024/2010)"*. This
...
fall back to the plain ISO mdoc presentation on User devices that do not support ZKPs.

Note (informative): Zero-Knowledge Proof generation depends on platform support. On User devices without this support, the fallback mechanism (Section 4 and Annex A) applies. The current platform support position is set out in [Annex B](annexes/annex-B/annex-B-zkp.md), Section B.4.

An individual IETF draft has been submitted, and an open-source
